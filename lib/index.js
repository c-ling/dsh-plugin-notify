 ...
 * system channel (macOS osascript / Linux notify-send, optional afplay sound)
...
export const DEFAULT_CONFIG = Object.freeze({
  ...
  browser: Object.freeze({ enabled: true, toast: true }),
...
/** Best-effort native toast: macOS osascript / Linux notify-send. */
export function systemNotify(title, body, execImpl = runExecFile) {
...
/** macOS system sound; no-op failure elsewhere. */
export function playSystemSound(execImpl = runExecFile) {
  if (platform() !== "darwin") return Promise.reject(new Error("仅 macOS 支持系统提示音"));
  ...
  const handlers = {
    ...
    async postTest(req, res) {
      try {
        ...
        const impls = {
          system: (systemCfg) => systemNotify(message.title, message.body).then(() => {
        ...
        if (channel === "system") {
          ...
          // Test sound follows the explicit request, else the saved toggle.
          const wantSound = typeof body.sound === "boolean" ? body.sound : current.system.sound;
          if (wantSound) await playSystemSound().catch(() => {});
