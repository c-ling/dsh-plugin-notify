      ...
      function handle(snapshot) {
        ...
        if (prev.initialized) {
          if (prev.running && !running && cfg.triggers && cfg.triggers.turnEnd !== false) {
            fireBrowserNotification({ title: format(t, "nav"), body: "任务回合已结束。" }, toastStore, { native: showNative, tag: "dsh-plugin-notify-turn-" + Date.now().toString(36) });
          }
          for (var pendingItem of pending) {
            ...
            }, toastStore, { native: showNative, tag: "dsh-plugin-notify-pending-" + pendingItem.key });
          }
        }
