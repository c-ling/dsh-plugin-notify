 ...
 *     toggles (browser banner/system/Feishu/DingTalk/WeCom/generic webhooks),
 *     secret inputs, and per-channel test buttons.
 *   - shell.overlay: the in-page toast banner stack (the browser channel's
 *     delivery surface; visible text that never depends on OS notification
 *     permission).
 *
  ...
  factory: (require) => {
    ...
    (function injectCss() {
      ...
      style.textContent = [
        ...
        ".dsh-plugin-notify-tag{display:inline-block;font-size:11px;line-height:1.2;padding:3px 8px;border-radius:999px;font-weight:500;background:rgba(128,128,128,.14);max-width:260px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}",
        ...
        ".dsh-plugin-notify-footer .dsh-plugin-notify-button{width:100%;background:var(--dsw-alias-brand-primary,#111417);color:#fff;border:none;border-radius:999px;padding:9px 12px;font-size:14px;font-weight:500;}",
        ".dsh-plugin-notify-footer .dsh-plugin-notify-button:hover{background:var(--dsw-alias-label-secondary,#2a2e33);}",
        ...
        ".dsh-plugin-notify-card{border:1px solid rgba(128,128,128,.28);border-radius:10px;padding:14px 16px;display:flex;flex-direction:column;gap:10px;position:relative;}",
        ...
        ".dsh-plugin-notify-switch{position:relative;flex:none;width:40px;height:22px;border-radius:999px;border:none;padding:0;background:rgba(128,128,128,.42);cursor:pointer;transition:background .15s ease;}",
        ".dsh-plugin-notify-switch:hover{background:rgba(128,128,128,.6);}",
        ".dsh-plugin-notify-switch-on{background:var(--dsw-alias-brand-primary,#111417);}",
        ".dsh-plugin-notify-switch-on:hover{background:var(--dsw-alias-brand-primary,#111417);}",
        ".dsh-plugin-notify-switch:disabled{opacity:.5;cursor:default;}",
        ".dsh-plugin-notify-switch-knob{position:absolute;top:2px;left:2px;width:18px;height:18px;border-radius:50%;background:#fff;box-shadow:0 1px 2px rgba(0,0,0,.3);transition:transform .15s ease;}",
        ".dsh-plugin-notify-switch-on .dsh-plugin-notify-switch-knob{transform:translateX(18px);}",
        ".dsh-plugin-notify-card h3{margin:0;font-size:14px;font-weight:600;}",
        ...
        ".dsh-plugin-notify-field > span{font-size:12px;opacity:.75;}",
        ".dsh-plugin-notify-input{width:100%;box-sizing:border-box;padding:6px 8px;border:1px solid rgba(128,128,128,.35);border-radius:6px;background:transparent;color:inherit;font:inherit;}",
        ".dsh-plugin-notify-textarea{width:100%;box-sizing:border-box;padding:6px 8px;border:1px solid rgba(128,128,128,.35);border-radius:6px;background:transparent;color:inherit;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:12px;resize:vertical;}",
        ".dsh-plugin-notify-hint{font-size:12px;opacity:.6;}",
        ".dsh-plugin-notify-actions{display:flex;gap:8px;align-items:center;flex-wrap:wrap;}",
        ".dsh-plugin-notify-button{padding:5px 12px;border:1px solid rgba(128,128,128,.4);border-radius:6px;background:transparent;color:inherit;font:inherit;cursor:pointer;}",
        ".dsh-plugin-notify-button:hover{border-color:rgba(128,128,128,.8);}",
        ".dsh-plugin-notify-button:disabled{opacity:.5;cursor:default;}",
        ".dsh-plugin-notify-error{color:#e5484d;font-size:12px;}",
        ".dsh-plugin-notify-ok{color:#46a758;font-size:12px;}",
        ".dsh-plugin-notify-kind{display:inline-flex;align-items:center;gap:4px;margin-right:10px;}",
        ".dsh-plugin-notify-generic-item{border:1px dashed rgba(128,128,128,.35);border-radius:8px;padding:10px;display:flex;flex-direction:column;gap:8px;}",
    ...
    var DICT = {
      zh: {
        ...
        kind_error: "出错",
        browser: "浏览器通知",
        browserHint: "页面可见时在右上角弹出文字横幅；离开页面时请配合系统通知使用",
        browserEnabled: "启用浏览器通知",
        browserToast: "页面内横幅",
        browserToastHint: "任务结束或等待确认时在右上角弹出文字横幅（不依赖系统通知权限）",
        testBrowser: "测试横幅",
        system: "系统通知",
        systemHint: "通过宿主机发送，浏览器关闭也能收到（macOS / Linux）",
        systemEnabled: "启用系统通知",
        systemSound: "系统提示音（macOS）",
        feishu: "飞书群机器人",
        dingtalk: "钉钉群机器人",
        wecom: "企业微信群机器人",
      ...
      en: {
        ...
        browserHint: "Shows a text banner in the top-right corner while the page is visible; use system notifications when you are away",
        ...
        systemHint: "Sent by the host process; works even with the browser closed (macOS / Linux)",
        systemEnabled: "Enable system notifications",
        systemSound: "System sound (macOS)",
    ...
    // ── browser notification (in-page banner only) ─────────────────────────

    /**
     * The browser channel is the in-page toast banner: it carries the text
     * while the page is visible and never depends on OS notification
     * permission. The system channel covers the away-from-page case.
     */
    function fireBrowserNotification(message, toastStore) {
    ...
    function startEngine(ctx, store, toastStore, t) {
      ...
      function handle(snapshot) {
        ...
        if (!cfg.browser || cfg.browser.enabled === false) return;
        ...
        if (prev.initialized) {
          if (prev.running && !running && cfg.triggers && cfg.triggers.turnEnd !== false && cfg.browser.toast !== false) {
            fireBrowserNotification({ title: format(t, "nav"), body: "任务回合已结束。" }, toastStore);
          }
          for (var pendingItem of pending) {
            ...
            if (cfg.browser.toast === false) break;
    ...
    function NotifySection(props) {
      ...
      function testBrowser() {
        if (draft?.browser?.toast !== false && toastStore) {
          fireBrowserNotification({ title: format(t, "nav"), body: "这是一条浏览器测试横幅。" }, toastStore);
        }
        setTestResults(function (prev) {
          var next = Object.assign({}, prev);
          next.browser = { status: "ok" };
          return next;
        });
      }
      ...
      return el("div", { className: "dsh-plugin-notify" },
        ...
        el(Card, {
          ...
          el("div", { className: "dsh-plugin-notify-actions" },
            el("button", { className: "dsh-plugin-notify-button", onClick: testBrowser }, t("testBrowser")),
        ...
        el(Card, {
          ...
          el("div", { className: "dsh-plugin-notify-actions" },
            el("button", { className: "dsh-plugin-notify-button", onClick: function () { runTest("system", undefined, { sound: system.sound }); } }, t("test")),
